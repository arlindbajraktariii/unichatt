...
const MessagesPage = () => {
  ...
  if (searchTerm) {
    ...
    filteredMessages = filteredMessages.filter(message => message.content.toLowerCase().includes(term) || message.sender_name.toLowerCase().includes(term));
  }
  ...
  return <div className="container mx-auto py-6 max-w-7xl bg-white">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="w-full md:w-auto flex flex-col md:flex-row gap-2">
          <div className="relative w-full md:w-[250px]">
            ...
            <Input placeholder="Search messages..." className="pl-8" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
          ...
          <Select value={channelFilter} onValueChange={setChannelFilter}>
            ...
            <SelectContent>
              ...
              {activeChannels.map(channel => <SelectItem key={channel.id} value={channel.id}>
                  {channel.name}
                </SelectItem>)}
            </SelectContent>
          </Select>
      ...
      <Tabs defaultValue={filter} value={filter} onValueChange={setFilter} className="mb-6">
      ...
      <div className="bg-white rounded-lg border shadow-sm min-h-[500px]">
        <div className="max-h-[calc(100vh-250px)] overflow-y-auto">
          <MessageList messages={filteredMessages} filter={filter !== "all" ? filter : undefined} channelId={channelFilter !== "all" ? channelFilter : undefined} />
        </div>
      </div>
    </div>;
};
