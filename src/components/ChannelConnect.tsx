...
const channelOptions = [{
  ...
  value: "teams",
  label: "Microsoft Teams (Coming Soon)"
}, {
  value: "gmail",
  ...
  return <Card className="max-w-md mx-auto">
      <CardHeader>
        <div className="flex items-center">
          {onBack && <Button variant="ghost" size="icon" className="mr-2" onClick={onBack}>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Select value={channelType} onValueChange={(value: ChannelType | "") => {
          ...
          setError("");
            ...
            <SelectContent>
              {channelOptions.map(option => <SelectItem key={option.value} value={option.value as ChannelType}>
                  {option.label}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        {showNameField && <div className="space-y-2">
            <Label htmlFor="channel-name">Connection Name</Label>
            <Input id="channel-name" placeholder="e.g., Work Slack, Personal Gmail" value={channelName} onChange={e => setChannelName(e.target.value)} />
        ...
        {(channelType === "slack" || channelType === "discord") && <Alert className="bg-amber-50 text-amber-800 border-amber-200">
            <InfoIcon className="h-4 w-4" />
      ...
      <CardFooter className="flex justify-between">
        ...
        <Button onClick={handleConnect} disabled={isConnecting} className="bg-zinc-950 hover:bg-zinc-800 text-white">
          {isConnecting ? <>
